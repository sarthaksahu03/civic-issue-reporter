...
const Footer: React.FC<{ withSidebarPadding?: boolean }> = ({ withSidebarPadding = true }) => {
  const year = new Date().getFullYear();
  return (
    <footer className="mt-10 border-t border-slate-200 dark:border-slate-700 bg-surface/60 dark:bg-surface-dark/60">
      <div className={`max-w-7xl mx-auto px-4 py-6 flex flex-col sm:flex-row items-center justify-between text-sm ${withSidebarPadding ? 'md:pl-72' : ''} transition-[padding] duration-300 ease-in-out`}>
        <p className="text-slate-600 dark:text-slate-300">© {year} CivicEyes. All rights reserved.</p>
